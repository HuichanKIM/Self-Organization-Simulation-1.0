...
var
  _BaseR, _BaseG, _BaseB: Byte;
  _Data: TBitmapData;
  _InvZoom: Double;
  _LocalMode: TFractalMode;
  _LocalJuliaCX, _LocalJuliaCY: Double;
  _LocalMaxIter: Integer;
  _LocalWidth, _LocalHeight: Integer;
  _LocalCenterX, _LocalCenterY: Double;
begin
  _BaseR := 22; _BaseG := 37; _BaseB := 61;
  _InvZoom := 4.0 / (FHeight * FZoom);

  // Local copies for thread-safety in parallel loop
  _LocalMode := FMode;
  _LocalJuliaCX := FJuliaCX;
  _LocalJuliaCY := FJuliaCY;
  _LocalMaxIter := FMaxIterations;
  _LocalWidth := FWidth;
  _LocalHeight := FHeight;
  _LocalCenterX := FCenterX;
  _LocalCenterY := FCenterY;
  ...
  try
    ...
    begin
      ...
      begin
        if _LocalMode = fmMandelbrot then
        begin
...
begin
  ...
  // Trigger re-render only when parameters change
  ...
  // Draw the processed fractal buffer to the main canvas
